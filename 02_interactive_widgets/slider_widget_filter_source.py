...
df = pd.DataFrame(dict(average_grades=[7,8,10],
                             exam_grades=[6,9,8],
                             student_names=["Stephan","Helder","Juddy"]))
...
def filter_grades(attr,old,new):
	source.data = df[df['exam_grades'] > slider.value]
...
slider=Slider(start=df["exam_grades"].min()-1,end=df["exam_grades"].max()+1,value=8,step=0.1,title="Exam Grade: ")
